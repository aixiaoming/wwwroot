...
if ($_REQUEST['querytype']=="update" or $_REQUEST['querytype']=="delete") {
	$check = ruiqia_query_fetch("SELECT*FROM goods WHERE id='".$_REQUEST['editid']."'");
	...
	if ($_REQUEST['querytype']=="delete" and $_REQUEST['editid']!="") {
		$img = ruiqia_query_fetch("SELECT*FROM goods WHERE id='".$_REQUEST['editid']."'");
		...
		ruiqia_query("DELETE FROM goods WHERE id='".$_REQUEST['editid']."'");
		header('Location:published_goods.php');
	}
