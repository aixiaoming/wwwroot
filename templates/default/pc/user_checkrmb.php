  ...
  <div class="checkrmb_container">
    ...
    <div class="rmb">
      <div>
        ...
        <a href="user_check.php">
          <button
            style="width: 100px;height: 30px;background-color: #f58710;border:0px;font-size: 16px;color: #fff;border-radius: 4px;cursor: pointer;margin-top: 10px;">
            填写资料
          </button>
        </a>
      </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        function update_native_state() {
            ...
            $.get
            {
                'D:\wwwroot\bangbangdream\wwwroot\sdk/example\native_notify.php?id=$_POST['
                id
                '];', function (data) {
                    v = eval("(" + data + ")");
                    if{v.state == 'userparing'}
                    {
                        $(".bottom").html('微信购买成功');
                    }
                    if (v.state == 'success') {
