...
<!-- 这是修改删除的界面表单 -->
 ...
 <br/><label for="model"><a>* </a>品牌型号</label>
...
<!-- 这里是物品照片 -->
  ...
  <br/><br/><label for="title" id="title_1"><a>* </a>标题</label>
  [成色] <input type='text' name="title" id="title" value="$check['title']"/>
 
  <br/><label for="editor" style="height:200px !important; width:125px !important; float:left;">物品描述</label>
...
<!-- 这里是物品上传过的照片 -->
  ...
  <br/><br/><label for="title" id="title_1"><a>* </a>标题</label>
  [成色] <input type='text' name="title" id="title" value=""/>
...
$('#submitbutton').click(function(){
  if ($.trim($("#model").val())=="") {    inputcheck('#model',".select1_nav",'品牌型号不能为空');
  }else if($.trim($("#title").val())==""){    inputcheck('#title',"#price",'标题不能为空');
  // }else if($("#editor").val()==""){
  //   layer.tips('活动描述不能为空或空格','#editor');
  //  $("html,body").animate({scrollTop:$('#people').offset().top},400);
  }else if($.trim($("#school").val())==""){    inputcheck('#school',"#area",'所在高校不能为空');
  }else if($.trim($("#linkman").val())==""){    inputcheck('#linkman',"#school",'联系人不能为空');
