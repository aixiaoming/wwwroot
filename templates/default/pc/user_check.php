...
<div class="usercheck_container" style="min-height: 1400px;">
  ...
  <div class="check_container">
    ...
    <!-- <div class="picture"> -->
    <div class="img1">
      <img src="/data/idcard/$user['id'].jpg?rand=<!--{eval echo mt_rand(1000,9999);}-->"
           onerror=this.src="/data/idcard/idcard.png" id="userlogoimg"
           onclick="document.getElementById('fileupload1').click()">
    </div>
    <div class="img2">
      <img src="/data/student/$user['id'].jpg?rand=<!--{eval echo mt_rand(1000,9999);}-->"
           onerror=this.src="/data/idcard/student.png" id="userlogoimg2"
  ...
  <div class="check_container">
    ...
    <div class="img">
      <img src="/data/idcard/$user['id'].jpg?rand=<!--{eval echo mt_rand(1000,9999);}-->"
           onerror=this.src="/data/idcard/qiyez.png" id="userlogoimg"
           onclick="document.getElementById('fileupload1').click()" style="float: left;width: 340px;height: 220px;">
      <img src="/data/student/$user['id'].jpg?rand=<!--{eval echo mt_rand(1000,9999);}-->"
           onerror=this.src="/data/idcard/qiyef.png" id="userlogoimg2"
           onclick="document.getElementById('fileupload2').click()" style="float: left;width: 340px;height: 220px;">
      <img src="/data/lience/$user['id'].jpg?rand=<!--{eval echo mt_rand(1000,9999);}-->"
           onerror=this.src="/data/idcard/license.png" id="userlogoimg3"
           onclick="document.getElementById('fileupload3').click()" style="width: 340px;height: 220px;">
    </div>
    ...
    <form class="form" method="post" action="check.php">
      ...
      <label>联系邮箱</label>
      <input name="email" type="text" value="$user['email']"><br>
      <label style="margin-left:32px;">经办人姓名</label>
      <input name="license" type="text" value="$user['license']"><br>
      <label style="margin-left: -15px;">经办人身份证件号</label>
      <input name="idcard" type="text" value="$user['idcard']"><br>
  ...
  <input id="fileupload3" type="file" accept='image/jpeg' name="mypic" style="display:none;">
</div>
...
// alert($("#userlogoimg").attr('rel'));
            ...
            $("#myupload2").ajaxSubmit({
                ...
                success: function (data) {
                    if (data.type == 'warning') {
                        ...
                        $("#userlogoimg").fadeIn(300);
                        $("#userlogoimg2").attr("src", '/data/idcard/student.png');
                    }
                    else {
                        var url = data.pic + '?rand=' + Math.random();
    ...
    $(function () {
        ...
        $("#fileupload3").wrap("<form id='myupload3' action='/actionalert.php?lei=lience' method='post' enctype='multipart/form-data'></form>");
        ...
        $("#fileupload3").change(function () {
            $("#myupload3").ajaxSubmit({
                ...
                beforeSend: function () {
                    ...
                    $("#userlogoimg3").fadeOut(300);
                    $("#userlogoimg3").attr("src", '/data/idcard/load.png');
                },
                ...
                success: function (data) {
                    if (data.type == 'warning') {
                        ...
                        $("#userlogoimg3").fadeIn(300);
                        $("#userlogoimg3").attr("src", '/data/idcard/license.png');
                    }
                    else {
                        var url = data.pic + '?rand=' + Math.random();
                        $("#userlogoimg3").attr("src", url);
                        $("#userlogoimg3").fadeIn(300);
                    }
                    // window.location.href = "/user_check.php";
                ...
                error: function (xhr) {
                    swal('提交失败', '请刷新页面再试试', 'error');
                    $("#userlogoimg3").attr("src", '/data/userface/error.jpg');
                }
