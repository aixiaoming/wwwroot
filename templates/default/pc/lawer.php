...
<div class="rightlaw">
    <span class="top_span">帮帮律师团</span>
    ...
    <!--{loop $lawer $key}-->
		...
		<img src="$key['logoimg']">
			...
			<div class="right">
				...
				<label>办公地点：</label><div><label title="$key['address']">$key['address']</label></div><br>
				<label>邮箱：</label><label>$key['email']</label>
				<a href="lawerlist.php?id=$key['id']">　　更多信息>></a>
			</div>
			<!--{if $user['administrator']==1}-->
			<div class="set">
                <form method="post" action="law_team_edit.php?querytype=update">
                <input name="editid" type="hidden" value="$key['id']"/>
                <input type="submit" value="修改"  class="submit"/>
                </form>
                <form method="post" action="law_team_edit.php?querytype=delete">
                <input name="editid" type="hidden" value="$key['id']"/>
