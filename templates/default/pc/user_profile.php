...
<div class="side">
		...
		<ul id="ul">
		<li  style="border-left:3px solid #e96c1f;padding-left:47px;"><a href="user_profile.php"> 个人资料</a></li>
...
<div class="typeone_container">
	<span class="top_span">个人资料</span>
	<div class="bar">
		<label>修改头像</label>

	</div>
	<div class="img">
	  <label>点击图片上传头像（jpg格式，1M以下）</label>
    ...
    <div class="form">
        ...
        <!--{if $user['loginm']!=""}-->
		<label>　　手机</label>
		<input type="text" name="phone" value="$user['phone']" readonly="readonly"/><span>不可修改</span><br>
		<!--{/if}-->
		<!--{if $user['loginm']==""}-->
		<label>　　手机</label>
		...
		<!--{if $user['logine']!=""}-->
		<label>　　邮箱</label>
		<input type="text" name="email" value="$user['email']" readonly="readonly" /><span>不可修改</span><br>
		<!--{/if}-->
		<!--{if $user['logine']==""}-->
		<label>　　邮箱</label>
...
<div class="typeone_container">
    ...
    <form method="post" action="profile.php" class="form">
		...
		<label>　　地址</label>
		<input name="address" type="text" value="$user['address']"><br>
	    <div id="textarea"><label class="textarea">单位简介</label></div>
