...
<div class="right_container">
	...
	<table>
		<tr>
			<td><a href="/published_find_notice.php" class="type">失物招领</a></td>
    ...
    <!--{if $user['administrator']==1}-->
        ...
        <table>
        	<tr>
        		<td><a href="/published_find_notice.php" class="type">管理员个人发布</a></td>
        		<td><a href="/published_find_notice.php?type=admin">网站总发布</a></td>
        	</tr>
        </table>
        <!--{/if}-->
        <!--{if $type=="admin"}-->
        <table>
        	<tr>
        		<td><a href="/published_find_notice.php">管理员个人发布</a></td>
        		<td><a href="/published_find_notice.php?type=admin" class="type">网站总发布</a></td>
        	</tr>
        </table>
        <!--{/if}-->
        </div>
    ...
    <!--{if $find_notice==Array ( )}-->
    <div class="empty">您还没有在失物招领发布过信息哦</div>
    <!--{elseif $find_notice!=Array ( )}-->
    <!--{loop $find_notice $key}-->
        <div class="first">
           <!--  <div class="second">
                <span>信息号:</span>
                <span>$key['id']</span>
            ...
            <div class="third">
               <div class="img">
                ...
                <div id="title">
                <div class="title">
                <a href="show.php?list=$key['id']&clickid=9"><span>$key['title']</span></a><br>
                </div>
               ...
               <!--{eval $usercity=ruiqia_query_fetch("select * from area where id='".$city."'");}-->
                <!--{if $usercity['name']==''}-->
               <span>全国</span><span>-失物招领</span>
               <!--{/if}-->

               <!--{if $usercity['name']!=''}-->
               <span>$usercity['name']</span><span>-失物招领</span>
               <!--{/if}-->
                </div>
                </div>
            </div>
            <div class="four">
                <form method="post" action="find_notice_edit.php?querytype=update">
                <input name="editid" type="hidden" value="$key['id']"/>
                <input type="submit" value="修改" />
                </form>
                <form method="post" action="find_notice_edit.php?querytype=delete">
                <input name="editid" type="hidden" value="$key['id']"/>
