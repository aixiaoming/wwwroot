...
<div class="right_container">
    ...
    <table>
		<tr>
			...
			<td><a href="/published_active.php"  class="type">校园活动</a></td>
            <!--{if $user['test']==1}-->
    ...
    <!--{if $user['administrator']==1}-->
        ...
        <table>
        	<tr>
        		<td><a href="published_active.php" class="type">管理员个人发布</a></td>
        		<td><a href="published_active.php?type=admin">网站总发布</a></td>
        	</tr>
        </table>
        <!--{/if}-->
        <!--{if $type=="admin"}-->
        <table>
        	<tr>
        		<td><a href="published_active.php">管理员个人发布</a></td>
        		<td><a href="published_active.php?type=admin" class="type">网站总发布</a></td>
        	</tr>
        </table>
        <!--{/if}-->
        </div>
    ...
    <!--{if $active==Array ( )}-->
    <div class="empty">您还没有在校园活动发布过信息哦</div>
    <!--{elseif $active!=Array ( )}-->
    <!--{loop $active $key}-->
        <div class="first">
            <!-- <div class="second">
            ...
            <div class="third">
                ...
                <div id="title">
                <div class="title">
                <a href="show.php?list=$key['id']&clickid=7"><span>$key['title']</span></a><br>
                </div>
               ...
               <!--{eval $usercity=ruiqia_query_fetch("select * from area where id='".$city."'");}-->
                <!--{if $usercity['name']==''}-->
               <span>全国</span><span>-校园活动-</span><span>$key['class']</span>
               <!--{/if}-->
               <!--{if $usercity['name']!=''}-->
                <span>$usercity['name']</span><span>-校园活动-</span><span>$key['class']</span>
                <!--{/if}-->
                </div>
                </div>  
            </div>
            <div class="four">
                <form method="post" action="active_edit.php?querytype=update">
                <input name="editid" type="hidden" value="$key['id']"/>
                <input type="submit" value="修改" />
                </form>
                <form method="post" action="active_edit.php?querytype=delete">
                <input name="editid" type="hidden" value="$key['id']"/>
