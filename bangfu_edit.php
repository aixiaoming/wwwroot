...
$jianjie = ruiqia_query_fetch("SELECT*FROM bangfu WHERE id=1");
...
if ($_REQUEST['id']!="" and $_REQUEST['type']=="del") {
	...
	if ($user['administrator']!=1) {
		...
		$img = ruiqia_query_fetch("SELECT*FROM bangfu WHERE id='".$_REQUEST['id']."'");
		// unlink (".".$img['img1']);	

		ruiqia_query("DELETE FROM bangfu WHERE id='".$_REQUEST['id']."'");
