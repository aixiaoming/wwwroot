...
// 确认，这个用户的确是处于密码重置的状态，还要确认，的确是用户本人
  $token = $_POST['token'];

   if(!$token)
   {
     echo '{"title":"验证失败","text":"","type":"warning"}';
      exit();
   }
 
  $time = time();

  $user = ruiqia_query_fetch("SELECT * from user_find_pass WHERE user='$userid' AND token='$token' AND time>$time");
