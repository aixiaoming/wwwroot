...
$classname = ruiqia_query_column('SELECT name FROM class WHERE id="'.$class.'"');
...
if ($_REQUEST['querytype']=="update" or $_REQUEST['querytype']=="delete") {
	$check = ruiqia_query_fetch("SELECT*FROM parttime WHERE id='".$_REQUEST['editid']."'");
	...
	if ($check['userid']!=$_SESSION['userid'] and $user['administrator']!=1) {
		ruiqia_die('不好意思！您没有权限编辑或删除');
	}
	if ($_REQUEST['querytype']=="delete" and $_REQUEST['editid']!="") {
		$img = ruiqia_query_fetch("SELECT*FROM parttime WHERE id='".$_REQUEST['editid']."'");
		...
		ruiqia_query("DELETE FROM parttime WHERE id='".$_REQUEST['editid']."'");
			
        header('Location:published_parttime.php');
	}
