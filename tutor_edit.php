...
if ($_REQUEST['querytype']=="update" or $_REQUEST['querytype']=="delete") {
	$check = ruiqia_query_fetch("SELECT*FROM tutor WHERE id='".$_REQUEST['editid']."'");
	...
	if ($_REQUEST['querytype']=="delete" and $_REQUEST['editid']!="") {
		 ...
		 $img = ruiqia_query_fetch("SELECT*FROM tutor WHERE id='".$_REQUEST['editid']."'");
		...
		if ($img['img10']!='') {unlink (".".$img['img10']);	}
		 ...
		 ruiqia_query("DELETE FROM tutor WHERE id='".$_REQUEST['editid']."'");
