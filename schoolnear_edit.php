...
if ($_REQUEST['querytype']=="update" or $_REQUEST['querytype']=="delete") {
	$check = ruiqia_query_fetch("SELECT*FROM schoolnear WHERE id='".$_REQUEST['editid']."'");
	if ($check['area']=='0') {
		$areaname = '不限';
	}else{
	...
	if ($_REQUEST['querytype']=="delete" and $_REQUEST['editid']!="") {
		$img = ruiqia_query_fetch("SELECT*FROM schoolnear WHERE id='".$_REQUEST['editid']."'");
		...
		if ($img['img10']!='') {unlink (".".$img['img10']);	}
		 ...
		 ruiqia_query("DELETE FROM schoolnear WHERE id='".$_REQUEST['editid']."'");
       	
		 header('Location:published_schoolnear.php');
	}
